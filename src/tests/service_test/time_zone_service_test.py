...
class TestTimeZoneService(unittest.TestCase):
    ...
    def tearDown(self):
        self.time_zone_service.clear_time_zones()

    def test_user_time_zone_is_found_correctly(self):
        self.time_zone_service.add_user_time_zone(1234, "US/Eastern")
        tz = self.time_zone_service.get_user_time_zone(1234)
    ...
    def test_time_zone_is_found_correctly_by_id(self):
        tz_id = self.time_zone_service.add_user_time_zone(1234, "US/Eastern")
        tz = self.time_zone_service.get_time_zone_by_id(tz_id)
        self.assertEqual(tz.time_zone, "US/Eastern")

    def test_user_time_zone_is_edited_correctly(self):
        self.time_zone_service.add_user_time_zone(1234)
        self.time_zone_service.edit_user_time_zone(1234, "US/Eastern")
        tz = self.time_zone_service.get_user_time_zone(1234)
        self.assertEqual(tz.time_zone, "US/Eastern")

    def test_time_zone_is_edited_correctly_by_id(self):
        tz_id = self.time_zone_service.add_user_time_zone(1234)
        self.time_zone_service.edit_time_zone_by_id(tz_id, "US/Eastern")
        tz = self.time_zone_service.get_time_zone_by_id(tz_id)
        self.assertEqual(tz.time_zone, "US/Eastern")

    def test_user_time_zone_is_deleted_correctly(self):
        self.time_zone_service.add_user_time_zone(1234, "US/Eastern")
        self.time_zone_service.delete_user_time_zone(1234)
        tz = self.time_zone_service.get_user_time_zone(1234)
        self.assertEqual(tz.time_zone, "UTC")

    def test_time_zone_is_deleted_correctly_by_id(self):
        tz_id = self.time_zone_service.add_user_time_zone(1234, "US/Eastern")
        self.time_zone_service.delete_time_zone_by_id(tz_id)
        tz = self.time_zone_service.get_time_zone_by_id(tz_id)
