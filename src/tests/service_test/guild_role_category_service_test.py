...
class TestGuildRoleCategoryService(unittest.TestCase):
    ...
    def test_guild_role_categories_are_found_correctly(self):
        self.guild_role_category_service.add_guild_role_category(1234, "TEST")
        categories = self.guild_role_category_service.get_all_guild_role_categories(1234)
    ...
    def test_guild_role_categories_are_removed_correctly(self):
        ...
        self.guild_role_category_service.remove_guild_role_category(categories[0].db_id)
        categories = self.guild_role_category_service.get_all_guild_role_categories(1234)
        self.assertEqual(len(categories), 0)

    def test_all_guild_role_categories_are_removed_correctly(self):
        self.guild_role_category_service.add_guild_role_category(1234, "TEST")
        self.guild_role_category_service.add_guild_role_category(2345, "TEST")
        categories1 = self.guild_role_category_service.get_all_guild_role_categories(1234)
        categories2 = self.guild_role_category_service.get_all_guild_role_categories(2345)
        self.assertEqual(len(categories1), 1)
        self.assertEqual(len(categories2), 1)
        self.guild_role_category_service.remove_all_guild_role_categories(1234)
        categories1 = self.guild_role_category_service.get_all_guild_role_categories(1234)
        categories2 = self.guild_role_category_service.get_all_guild_role_categories(2345)
