...
class TestVerificationQuestionsDAO(unittest.TestCase):
    ...
    def test_guild_verification_questions_are_found_correctly(self):
        ...
        questions = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(1234))
        self.assertEqual(len(questions), 1)
        self.assertEqual(questions[0]["question"], "Test1?")

    def test_specific_verification_questions_are_found_correctly(self):
        asyncio.run(self.verification_questions_dao.add_verification_question(1234, "Test1?"))
        question = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(1234))
        question = asyncio.run(self.verification_questions_dao.get_verification_question(question[0]["id"]))
    ...
    def test_verification_questions_are_edited_correctly(self):
        ...
        asyncio.run(self.verification_questions_dao.edit_verification_question(questions[0]["id"], "Test2?", 1))
        questions = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(1234))
    ...
    def test_verification_questions_are_deleted_correctly(self):
        ...
        questions = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(1234))
        self.assertEqual(len(questions), 1)
        asyncio.run(self.verification_questions_dao.delete_verification_question(questions[0]["id"]))
        questions = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(1234))
    ...
    def test_guild_verification_questions_are_deleted_correctly(self):
        asyncio.run(self.verification_questions_dao.add_verification_question(1234, "Test1?"))
        asyncio.run(self.verification_questions_dao.add_verification_question(2345, "Test2?"))
        questions1 = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(1234))
        questions2 = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(2345))
        self.assertEqual(len(questions1), 1)
        self.assertEqual(len(questions2), 1)
        asyncio.run(self.verification_questions_dao.delete_guild_verification_questions(1234))
        questions1 = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(1234))
        questions2 = asyncio.run(self.verification_questions_dao.get_all_guild_verification_questions(2345))
